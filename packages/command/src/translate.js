...
class Translate {
  ...
  readFile() {
    ...
    if (!needTranslateLangueges) {
      needTranslateLangueges = languages.filter(item => item.name !== 'zh-CN').map(item => item.name);
    }
  ...
  async translate() {
    ...
    for (let curLanguage in this.translateTable) {
      ...
      for (let item of list) {
        const formatChinese = word =>
          word.chinese.replace(/(\{\{(\@[0-9]+?)\}\})/g, ($1, $2, $3) => $3);
        ...
        const data = await this.requestTranslate(
          item.map(n => formatChinese(n)).join('\n'),
          info.name
        );
      ...
      fs.writeFileSync(curFilePath, await prettierJs(file), { encoding: 'utf-8' });
    }
  }
